...
start_synod_member(UniqueId) ->
  Pid = spawn(fun() ->  synod_member(UniqueId,{0,null},null,0,null,nul) end),
    ...
    {vote,{NewNum,NewUid},From} ->
      if  
        ...
        BUid == null ->
          ...
          synod_member(Uid,{BNum,BUid},V,NewNum,L,Status);
        NewNum > High ->
    ...
    {beginballot,{NNum,NUid},Value,From} ->
      if  
        NNum == High ->
          ...
          synod_member(Uid,{NNum,NUid},Value,High,L,Status);
        true ->
          From ! ignored,
          synod_member(Uid,{BNum,BUid},V,High,L,Status)
      end;
    {success,Value} ->
      resolved_synod_member(Uid,Value);
    disable ->
      disabled_synod_member(Uid,{BNum,BUid},V,High,L,Status);
    {list,List,From} ->
      From ! ok, 
...
send_fake_lastvote(ReceiverUniqueId,FakeSenderUniqueId,PrevBallot,PrevVote,NewBallot) ->
  ReceiverUniqueId ! {lastvote,FakeSenderUniqueId,PrevBallot,PrevVote,NewBallot},
  ok.
